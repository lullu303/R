...
# mpg data를 data 프레임 형태로 변환해서 mpg 변수 data에 저장
mpg <- as.data.

View(mpg)

#mpg data의 앞부분을 ㅗ학인한다 (20line 확인)
head(mpg,20)
...
# total 변수의 요약 통계량을 확인하고 히스토그램을 출력하여 total 변수의 구간별 빈도수를 출력
summary(new_mpg$total)
hist(new_mpg$total)

# 히스토그램을 확인해서 빈도수가 높은 구간을 연비의 합격으로 규정하고 해당 구간
# total 연비가 낮은 자동차는 fail 같거나 높은 자동차는 pass 값을 갖는 test 변수

new_mpg$test <- ifelse(new_mpg$total >= 20, 'pass', 'fail')

# 추가가 제대로 이루어졌는지 mpg 데이터의 일부르 ㄹ확인
head(new_mpg,2)
...
new_mpg <- new_mpg %>%  mutate(grate2 = ifelse(total >=30, 'A',
                                       ifelse(total >=20,'B',
                                              ifelse(total>=15, "C","D"))))
