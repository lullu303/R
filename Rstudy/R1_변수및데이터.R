...
# 변수명 <- 연산식 : 오른쪽의 연산식을 계산해서 결과를 왼쪽 변수에 저장
# <- / = (할당 연산자, 대입 연산자)
x <- 10
x = 100
x # 변수 저장 값 조회
...
a
# 1 이외의 수부터 시작하면 안됨
b <- seq(10:15) # 1 2 3 4 5 6 이 생성됨
b

# by = +/- 간격
x4 <- seq(1,10, by=4) # 1부터 10까지 4간격의 숫자 생성
x4 # 1 5 9
...
# factor형 변수는 연산 불가
var_n + 2
var_f + 2 # 연산은 의미가 없음(factor형 이므로)
#[1] NA NA NA NA NA
# Warning message:
...
# 함수 : 작업 수행(기능)
# 작업을 수행하고 결과값을 반환하도록 만들어진 코드 집합
# 변수 : 값을 저장
