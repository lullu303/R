...
# 지역별 차이가 있는지 평균 검정을 통해 확인(특정 2개의 지역)

# 전 지역의 미세먼지에 대한 비교 그래프
# 미세먼지 농도의 평균을 활용해서 서울시 구별의 차이을 단계그래프로 표시 
...
# raw 데이터 읽어오기
data<- read_excel('data/period.xlsx',skip=3)
View(data)
dim(data)
str(data)
ls(data)
...
# t검정을 통한 평균 차이 검정
# t검정 : 많이 알려진 평균 차이 검정의 통계적 모령, t분포 활용
# t.test(data=데이터세트, 변수2(관측치) ~ 변수1(기준), var.equal=T)
# var.equl= : 두 비교 집단의 분산동질성 여부(같다라고 가정)
...
# area 컬럼에서 NA 빈도수-지자체명이 없으면 데이터 사용 불가
# 행제거(지자체와 상관없는 관측일의 평균과 전체 평균 데이터 이므로
# 사용할 필요 없음)
table(is.na(dustdata_NA$area))

# finedust 컬럼(실제 관측치) NA의 빈도수
# 미세먼지 관측이 안된 데이터가 12개 - 비율이 크지 않으면
# 제거하지 않고 대표값으로 대체 해서 사용
...
# 특정 df의 일부 데이터로 그래프 그릴려면
# data=일부데이터
# x,y 축은 부분 매핑이 불가함
ggplot(dustdata_w[20:30,],aes(x=yyyymmdd, y=성북구)) +
  geom_line(group=1,colour='red') +
  theme(axis.text.x=element_blank(),
        axis.line.x=element_blank(),
        axis.ticks.x = element_blank())
...
# 여러 구의 미세먼지 농도 시각화는 wide형보다 long형 데이터가
# 더 유용함
...
View(dustdata_w)
ls(dustdata_w)
# 날짜 컬럼이 있으므로 제외 후 평균계산
mean(dustdata_w[,-1])

# 각 구의 평균을 wide 형으로 구함
...
# 지도데이터의 코드를 활용해서 수치데이터에 구별 행정코드 추가

s_map_code <- seoul_map %>% select(code,name)
View(s_map_code)

# 행정코드와 자치구명이 중복되어 여러번 나타나므로 자치구별 1개씩말
# 추출
