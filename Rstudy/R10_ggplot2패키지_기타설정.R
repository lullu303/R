...
# area 그래프로 영역을 설정
# 데이터 추이 값을 추가
subway <- read.csv("data/1-4호선승하차승객수_MAC용.csv")
head(subway)

# 노선번호별 승차인원이 많은 순으로 영역 그래프 생성
...
# 기준열의 순서를 factor로 변환 후 직접 설정해야 함
# 순서 자체가 사전적 의미가 필요하면 factor화 필요 없음.
...
# breaksdp 대입되는 값의 개수와 labels에 대입되는 값의 개수가 동일해야 함

g1 + scale_x_continuous(breaks=seq(0,2000,by=200),
                        labels= paste(seq(0,2000,by=200),'age')) +
  scale_y_continuous(breaks=seq(0,300, by=30)) +
  theme(axis.text.x = element_text(colour = 'red', size=10, angle = 90),
        axis.text.y = element_text(colour = 'blue', size=10))

# theme 
# 축글자 axis.text / axis.text.x / axis.text.y
...
# 개인저축율은 날짜가 지남에 따라 0.000544 만큼 감소
